...
> ℹ️ **Nota de consistencia interna:** `LLBG` está excluido aquí por regla explícita, aun cuando el problema de nomenclatura de `LLBG` (dos descripciones para el mismo ICAO) ya fue corregido en Power Query (ver `power_query_m.md`). El motivo de mantener la exclusión está documentado en el case study: el dataset no registra tráfico de `LLBG` antes de 2018, por lo que ya sería descartado automáticamente por el criterio de serie histórica completa (mínimo 10.000 vuelos en 2017). La regla explícita simplemente documenta la intención de forma legible.
>
> Dato corroborante: `GMMN` (Mohammed V, Casablanca) —uno de los aeropuertos que dominaba el ranking distorsionado original junto a `LTFM` y `LLBG`— tampoco tiene registros antes de 2018, y por lo tanto queda excluido del cálculo de crecimiento sin necesidad de agregarlo a la lista explícita. Es una buena validación de que el criterio numérico de elegibilidad captura anomalías reales incluso cuando no están hardcodeadas.
